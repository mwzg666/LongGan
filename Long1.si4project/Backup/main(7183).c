...
void InitParam()
{
	...
	SysRunState.stParam.s_SysParam.Z1 = 2500-600;              //甄别器阈值1
	...
	SysRunState.stParam.s_SysParam.DiYaCanshuA = 0.63;         //低量程通道校准因子
	SysRunState.stParam.s_SysParam.DiYaCanshuB = 0.00019;      //低量程通道校准因子
	SysRunState.stParam.s_SysParam.DiYaCanshuC = 0.83;         //1;低量程通道校准因子
...
void TimerTask()
{
    ...
    if(Timer0Cnt)
    {
        ...
        if(Time1s >= 1000)                      //100*10=1000ms
        {
            ...
            Pluse4Cnt = 200;//((u32)LowOverFlowFlag4*65536)<<8 | (u32)(T4H*256+T4L) ;
            T4R = 0; 
            T4H = 0;
            T4L = 0;
            T4R = 1;            
